...
void init_mydata(){
	//mipws afta ta pragmata theloun arxikopoihsh
	   A = new int[n + 1];
    inX = new int[n + 1];
	inA = new int[n + 1];
...
void packing_arborescences(){
   ...
   while(1){
	  ...
	  ntrees = construct_trees(1);	  			//initialize T to a complete (k -1) intersection 
	
	  find_tree(); 								//output arborescence A  
	 ...
	 //release_used_edges();
	 G_minus_A();								// remove the edges of the arborescence from the graph	
  }
}
